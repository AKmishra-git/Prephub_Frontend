...
export default async function DbmsVideosPage() {
  // 🎥 fetch DBMS videos
  const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/prep/videos/dbms/dbms`, {
    cache: "no-store",
  })

  const data = await res.json()
  const dbmsVideos = data?.data ?? data ?? []
    ...
    <div className="mx-auto max-w-6xl px-4 py-12">
      ...
      <div className="mb-10 flex flex-col gap-6 md:flex-row md:items-start md:justify-between">
        ...
        <div>
          ...
          <h1 className="text-3xl font-bold text-foreground">DBMS</h1>

          <p className="mt-2 text-muted-foreground">
            Database Management Systems concepts like SQL, normalization, indexing, transactions, and more.
          </p>
        </div>
        ...
        <div className="relative min-w-[240px] max-w-[340px] w-full overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-br from-white/10 to-white/5 p-4 backdrop-blur-xl shadow-lg">
          ...
          {/* 🔥 this handles cookies + fetching */}
          <ProgressClient subject="dbms" />
        </div>
